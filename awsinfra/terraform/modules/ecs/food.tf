module "food" {
  ...
  command                         = "server"
  component_ecr_url               = "${data.aws_caller_identity.current.account_id}.dkr.ecr.${var.region}.amazonaws.com/${var.project_name}-food-${var.env}"
  component_image_tag             = var.food_image_tag
  container_cpu                   = 2048
